module Skirmish::CityList
  ...
  def random_cities(min_population, angle)
    cities = select_cities(min_population)
    random_city = cities[SecureRandom.random_number(cities.length)]
    citylist = select_cities(min_population).select do |city|
  ...
  def select_us_cities(population_minimum)
  ...
  def select_global_cities(population_minimum)
